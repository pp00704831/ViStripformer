...
for _, video in enumerate(os.listdir(data_path)):
    ...
    if not os.path.isdir(output_dir):
        os.mkdir(output_dir)
    input_frames = sorted(glob.glob(os.path.join(data_path, video, 'Blur', 'RGB', "*")))
    ...
    for idx, clip_idx in enumerate(clip_dir):
        ...
        print('Video:{} ({}), Frames:{} to {}, Runtime:{:.4f}'.format(video, video_length, name_list[0], name_list[-1], runtime / out.shape[1]))

print('Save at:', save_dir)
print('Average Runtime:{:.4f}'.format(total_runtime / total_data))
