...
out_path = './out/Video_Stripformer_RainSynComplex25'
gt_path = './dataset/RainSynComplex25/test/GT'
...
for video in os.listdir(out_path):
    ...
    for idx, image_name in enumerate(os.listdir(out_video_dir)):
        count += 1
        gt_name = image_name.split('.')[0] + '.jpg'
        out_image_dir = os.path.join(out_video_dir, image_name)
        gt_image_dir = os.path.join(gt_video_dir, gt_name)
        ...
        ssim = calc_ssim(image.astype(np.uint8), gt_image.astype(np.uint8))
