...
# hyperparameters
frame_num = 16
data_path = './dataset/GOPRO_Large/test'
model_name = './weights/Deblur/Video_Stripformer_Plus_GoPro.pth'
save_dir = './out/Video_Stripformer_Plus_GoPro'
...
for _, video in enumerate(os.listdir(data_path)):
    ...
    if not os.path.isdir(output_dir):
        os.mkdir(output_dir)
    input_frames = sorted(glob.glob(os.path.join(data_path, video + '/blur', "*")))

    video_length = len(input_frames)
    total_data += video_length
    ...
    for idx, clip_idx in enumerate(clip_dir):
        ...
        print('Video:{} ({}), Frames:{} to {}, Runtime:{:.4f}, Avg Runtime:{:.4f}'.format(video, video_length, name_list[0], name_list[-1], runtime / out.shape[1], total_runtime / count))

print('Save at:', save_dir)
print('Total Images:{:.4f}, Average Runtime:{:.4f}'.format(total_data, total_runtime / total_data))
